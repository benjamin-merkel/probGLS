#' probabilistic algorithm for geolocation data (experimental version)
...
#' @param cond data.frame containing raw conductivity data , NULL if no conductivity data is available (algorithm will assume that the logger was always in water and not in the Baltic Sea)
#' @param cond.threshold expected two values: first values = upper threshold, i.e. not in the Baltic Sea; second value = lower threshold, i.e. animal on land below this threshold; animal assume to be in the Baltic Sea (i.e. within the box: lon>14 & lon<33.5 & lat>51.4 & lat<66.2) inbetween the two thresholds
...
for(ts in steps){
  ...
  if(length(grr$dtime[grr$step == ts])>0){
    ...
    # if no tag.SST available or conductivity data puts animal on land only use speed weighing----
    if(!is.na(gr3$tag.sst[1])) gselect  <- lapply(gr2,function(x) x$wspeed * x$wsst)
    if( is.na(gr3$tag.sst[1]) | obs.cond < cond.threshold[2]) gselect  <- lapply(gr2,function(x) x$wspeed)
